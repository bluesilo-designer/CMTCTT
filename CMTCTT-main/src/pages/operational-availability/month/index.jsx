...
import { ChevronRight, ArrowLeft, Clock, AlertCircle, BarChart2 } from "lucide-react";
...
const PER_PAGE = 10;
export function OperationalAvailabilityMonth({ monthId, onNavigate }) {
    ...
    const filtered = useMemo(() => {
        if (!record)
            return [];
        return record.bookings.filter((b) => !search ||
            b.name.toLowerCase().includes(search.toLowerCase()) ||
            b.bookingRef.toLowerCase().includes(search.toLowerCase()));
    }, [record, search]);
    const totalPages = Math.max(1, Math.ceil(filtered.length / PER_PAGE));
    const indexedPaginated = useMemo(() => {
    ...
    const columns = useMemo(() => [
        {
            id: "no",
            header: () => "No",
            cell: (info) => (<div onClick={() => onNavigate?.(`/operational-availability/${monthId}/${info.row.original.id}`)} className="text-sm text-gray-500 cursor-pointer">
        ...
        },
        {
            id: "bookingRef",
            header: () => "Booking Ref",
            cell: (info) => (<div onClick={() => onNavigate?.(`/operational-availability/${monthId}/${info.row.original.id}`)} className="cursor-pointer">
        ...
        },
        {
            id: "name",
            header: () => "Booking Name",
            cell: (info) => (<div onClick={() => onNavigate?.(`/operational-availability/${monthId}/${info.row.original.id}`)} className="text-sm font-medium text-gray-800 hover:text-brand-primary transition-colors cursor-pointer">
        ...
        },
        {
            ...
            cell: (info) => (<div onClick={() => onNavigate?.(`/operational-availability/${monthId}/${info.row.original.id}`)} className="text-sm text-gray-700 font-mono cursor-pointer">
        ...
        },
        {
            id: "dt",
            header: () => "Down Time (DT)",
            cell: (info) => (<div onClick={() => onNavigate?.(`/operational-availability/${monthId}/${info.row.original.id}`)} className={cn("text-sm font-mono cursor-pointer", info.row.original.dt !== "0m 0s" ? "text-red-500 font-semibold" : "text-gray-400")}>
    ...
    return (<div className="flex-1 overflow-auto bg-gray-50">
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => onNavigate?.("/operational-availability")} className="p-2 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 text-gray-500 hover:text-brand-primary transition-colors">
            <ArrowLeft size={16}/>
          </button>
          <div>
            <h1 className="text-xl font-semibold text-brand-primary">{record.month}</h1>
            <p className="text-xs text-gray-400 mt-0.5">Operational availability breakdown by booking</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
                icon: <BarChart2 size={18} className={good ? "text-green-500" : "text-red-500"}/>,
            ...
            },
            {
                ...
                sub: `${devicesUsed} device sessions`,
                mono: false,
            },
              ...
              <div>
                ...
                <p className={cn("text-2xl font-bold mt-0.5 leading-none", card.mono
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-700">
              Bookings <span className="text-gray-400 font-normal">({record.bookings.length})</span>
            </p>
            <InputCustom type="text" placeholder="Search booking..." value={search} onChange={(e) => {
