...
export function MonthRangePicker({ value, onChange, onClose }) {
    // step 1 = pick "from", step 2 = pick "to"
    ...
    const handleApply = () => {
        if (hasFrom && hasTo) {
            onChange({
                fromYear: pending.fromYear,
                fromMonth: pending.fromMonth,
                toYear: pending.toYear,
    ...
    return (<div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-xl border border-gray-200 shadow-xl w-72 p-4 select-none">
      {/* Header */}
      ...
      {(hasFrom || hasTo) && (<div className="text-xs text-brand-primary font-medium bg-red-50 rounded-lg px-3 py-1.5 mb-3">
      ...
      {/* Year navigation */}
      <div className="flex items-center justify-between mb-3">
        <button onClick={() => setYear((y) => y - 1)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
          <ChevronLeft size={14}/>
        </button>
      ...
      <div className="grid grid-cols-3 gap-1.5 mb-4">
        {MONTH_ABBR.map((name, m) => {
            ...
            return (<button key={name} onClick={() => handleMonthClick(m)} className={cn("py-2 rounded-lg text-xs font-medium transition-all", isSelected
      ...
      <div className="flex gap-2">
        <button onClick={handleClear} className="flex-1 py-2 rounded-lg text-xs font-semibold border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
          Clear
        </button>
        <button onClick={handleApply} disabled={!hasFrom || !hasTo} className={cn("flex-1 py-2 rounded-lg text-xs font-semibold transition-colors", hasFrom && hasTo
