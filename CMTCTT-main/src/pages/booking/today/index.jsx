...
export function TodaysBooking({ onNavigate }) {
    ...
    return (<div className="flex-1 overflow-auto bg-gray-50">
      <div className="p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 md:mb-6 gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            <Button type="outline" onClick={() => onNavigate("/bookings/list")} className="px-4 py-2 text-sm font-medium w-auto">
              View All Bookings
            </Button>
            <Button type="default" className="px-4 py-2 text-sm font-medium bg-brand-primary hover:bg-brand-primary-hover w-auto">
              Issue Assets
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6 mb-4 md:mb-6">
          {SUMMARY_CARDS.map((card) => (<div key={card.label} className="bg-white rounded-lg border border-gray-200 p-4 flex items-center gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto -mx-4 md:mx-0">
            <div className="relative" style={{ minWidth: "960px" }}>
              ...
              <div className="flex">
                ...
                <div className="w-24 flex-shrink-0 border-r border-gray-200">
                  ...
                  {TIMELINE_HOURS.map((hour) => (<div key={hour} className="h-24 border-b border-gray-100 flex items-start justify-center pt-2">
                ...
                <div className="flex-1 relative">
                  ...
                  <div className="relative">
                    {TIMELINE_HOURS.map((hour) => (<div key={hour} className="h-24 border-b border-gray-100"/>))}
            ...
            return (<div key={booking.id} onClick={() => setSelectedBooking(booking)} className={cn("absolute left-4 right-4 border-2 rounded-lg p-3 cursor-pointer transition-all hover:shadow-md", getStatusColor(booking.status))} style={{
                    top: `${topPercent}%`,
      ...
      {selectedBooking && (<BookingDetailModal booking={selectedBooking} onClose={() => setSelectedBooking(null)} onNavigate={onNavigate}/>)}
    </div>);
}
