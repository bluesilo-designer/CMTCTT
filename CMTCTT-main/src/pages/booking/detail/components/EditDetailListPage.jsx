...
const DragHandle = () => (<span className="text-gray-300 cursor-grab flex-shrink-0">
...
export function EditDetailListPage({ onBack, onConfirm }) {
    return (<div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {EDIT_DETAILS.map((detail) => (<div key={detail.label} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                ...
                if (row.type === "closed")
                    return (<div key={laneIdx} className="flex items-center gap-2 px-3 py-2.5 bg-red-50">
                      <DragHandle />
                ...
                if (row.type === "available")
                    return (<div key={laneIdx} className="flex items-center gap-2 px-3 py-2.5 bg-green-50">
                      <DragHandle />
                      <span className="flex-1 text-xs font-medium text-green-600">Available</span>
                      <span className="text-xs text-gray-400 flex-shrink-0">Lane {laneNum}</span>
                    </div>);
                return (<div key={laneIdx} className="flex items-center gap-2 px-3 py-2.5 hover:bg-gray-50 transition-colors">
                      <DragHandle />
