...
export function AssetType() {
    const [assetTypeList, setAssetTypeList] = useState(initialAssetTypes);
    ...
    const filtered = assetTypeList.filter((t) => !searchQuery || t.assetType.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3">
            {selected.size > 0 && (<Button onClick={handleDeleteSelected} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors">
                <Trash2 size={14}/>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          <AssetTypeTable data={sorted} currentPage={currentPage} selected={selected} sortField={sortField} sortDir={sortDir} onPageChange={setCurrentPage} onSort={handleSort} onToggleSelect={toggleSelect} onToggleAll={toggleAll}/>
        </div>
      </div>

      {/* Modals — parent controls mounting */}
      {showAddModal && (<AddAssetTypeModal onClose={() => setShowAddModal(false)} onAdd={handleAdd}/>)}
    </div>);
}
