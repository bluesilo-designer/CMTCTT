...
export function AssetCategory() {
    ...
    const filtered = assetCategories.filter((c) => !searchQuery || c.assetCategory.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button className="px-4 py-2 text-sm bg-brand-primary text-white rounded-md hover:bg-brand-primary-hover font-medium" onClick={() => setShowAddModal(true)}>
              + Add Asset Category
            </Button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          <AssetCategoryToolbar searchQuery={searchQuery} onSearchChange={handleSearchChange}/>
          <AssetCategoryTable data={sorted} currentPage={currentPage} selected={selected} sortField={sortField} sortDir={sortDir} onPageChange={setCurrentPage} onSort={handleSort} onToggleSelect={toggleSelect} onToggleAll={toggleAll} onEditTarget={setEditTarget} onDeleteRow={handleDeleteRow}/>
        </div>
      </div>

      {/* Modals — parent controls mounting */}
      {showAddModal && <AddAssetCategoryModal onClose={() => setShowAddModal(false)}/>}
      {editTarget && (<EditAssetCategoryModal categoryName={editTarget.name} currentStatus={editTarget.status} currentAlertMode={editTarget.alert} onClose={() => setEditTarget(null)}/>)}
    </div>);
}
