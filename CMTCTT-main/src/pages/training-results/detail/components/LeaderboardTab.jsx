...
function rankStyle(r) {
    if (r === 1)
        return { bg: "bg-amber-400", text: "text-white", label: "1st" };
    if (r === 2)
        return { bg: "bg-gray-400", text: "text-white", label: "2nd" };
    if (r === 3)
        return { bg: "bg-orange-600", text: "text-white", label: "3rd" };
    return { bg: "bg-gray-100", text: "text-gray-600", label: String(r) };
}
export function LeaderboardTab({ courseware, setCourseware }) {
    ...
    const totalPages = Math.max(1, Math.ceil(sorted.length / LEADERBOARD_PER_PAGE));
    ...
    const columns = useMemo(() => [
        {
            ...
            cell: (info) => {
                ...
                return (<div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-xs font-black", rs.bg, rs.text)}>
            {rs.label}
          </div>);
            },
        },
        ...
        },
        {
            header: "MPI (mm)",
            accessorKey: "mpi",
            cell: (info) => <span className="text-sm text-gray-600">{info.getValue().toFixed(2)}</span>,
        },
    ], []);
