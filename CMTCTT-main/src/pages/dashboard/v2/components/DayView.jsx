    ...
    return (<div className="bg-white rounded-xl border border-red-50/80">
      ...
      <div className="flex items-center gap-2 px-5 py-2.5 border-b border-slate-100 flex-wrap">
        {SESSION_PILLS.map(({ label, time, count, type }) => {
            ...
            return (<div key={type} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border", c.bg, c.text, c.border)}>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[900px] relative">
          ...
          <div className="flex border-b border-slate-100">
            ...
            {HOURS.map((h) => (<div key={h} className="flex-1 text-center text-xs text-slate-300 py-2 border-l border-slate-50">
                {h <= 12 ? `${h}AM` : `${h - 12}PM`}
              </div>))}
          </div>
          ...
          {STATIONS.map((station) => {
            ...
            return (<div key={station} className="flex border-b border-slate-50" style={{ minHeight: 64 }}>
                ...
                <div className="flex-1 relative">
                  {HOURS.map((_, i) => (<div key={i} className="absolute top-0 bottom-0 border-l border-slate-50" style={{ left: `${(i / totalHours) * 100}%` }}/>))}
                  {stationBookings.length === 0 ? (<div className="absolute inset-0 flex items-center justify-center text-xs text-slate-200">
                      Available
                    ...
                    return (<div key={bi} className={cn("absolute rounded-md px-2 py-1.5 text-xs font-semibold flex items-center gap-1.5 border", c.bg, c.text, c.border)} style={{ left: `${left}%`, width: `${width}%`, top: topOffset + 8, minHeight: 24 }}>
