...
import { STATIONS, weekData, weekDailyCounts, sessionColors, statusDot, WEEK_DAYS, WEEK_DATES, TODAY_IDX, } from "../constants";
    ...
    return (<div className="bg-white rounded-xl border border-red-50/80">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
        ...
        <div className="flex items-center gap-4 text-xs text-slate-400">
          {["Completed", "Ongoing", "Upcoming"].map((s) => (<div key={s} className="flex items-center gap-1.5">
              <div className={cn("w-2.5 h-2.5 rounded-sm", s === "Completed" ? "bg-emerald-200" :
      ...
      <div className="flex items-center gap-2 px-5 py-2.5 border-b border-slate-100">
        {["AM", "PM", "Full", "Hr", "Ad"].map((t) => {
            ...
            return (<button key={t} onClick={() => toggleFilter(t)} className={cn("flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border transition-colors", active ? `${c.bg} ${c.text} ${c.border}` : "bg-white text-slate-300 border-slate-200")}>
        ...
        <table className="w-full min-w-[800px]">
          <thead>
            <tr>
              ...
              {WEEK_DAYS.map((day, i) => (<th key={day} className={cn("py-2 text-center", i === TODAY_IDX ? "bg-red-50/50" : "")}>
          ...
          <tbody>
            ...
            return (<tr key={station} className="border-t border-slate-50">
                  ...
                  {WEEK_DAYS.map((day, di) => {
                    ...
                    return (<td key={day} className={cn("px-1.5 py-2 align-top", di === TODAY_IDX ? "bg-red-50/30" : "")}>
                        {cells.length === 0 ? (<div className="h-8 flex items-center justify-center text-slate-200 text-xs">–</div>) : (<div className="flex flex-col gap-1">
                            {cells.map((cell, ci) => {
                                const c = sessionColors[cell.type];
                                return (<div key={ci} className={cn("flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs font-semibold border", c.bg, c.text, c.border)}>
                                  <span>{cell.type}</span>
            ...
            <tr className="border-t border-slate-100 bg-slate-50/50">
              ...
              {weekDailyCounts.map((count, i) => (<td key={i} className={cn("py-2 text-center text-sm font-bold", i === TODAY_IDX ? "bg-red-50 text-red-500" : "text-gray-600")}>
                  {count}
                </td>))}
            </tr>
          </tbody>
