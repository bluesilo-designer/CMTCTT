...
import type { BookingRowsProps } from "../types";

export function BookingRows({ record }: BookingRowsProps) {
  ...
  return (
    <div className="bg-red-50/60 px-6 pb-2">
      {/* Sub-table header */}
      ...
      {record.bookings.map((booking) => (
        <div
          ...
          className="grid border-b border-gray-100 last:border-b-0 hover:bg-red-50/30 cursor-pointer transition-colors"
          style={{ gridTemplateColumns: "280px 160px 160px 1fr 1fr" }}
        >
          {/* Booking ID */}
          ...
          <div className="py-3 pr-3 align-top">
            <span className={cn(
              "text-sm font-mono",
              booking.dt !== "0m 0s" ? "text-red-500 font-semibold" : "text-gray-400"
            )}>
              {booking.dt}
            </span>
          </div>
              ...
              <div className="space-y-1">
                {booking.devices.map((device) => (
                  <DeviceCell
                    ...
                    total={device.totalUptime}
                    laneNames={device.lanes.map((l) => l.lane)}
                    laneValues={device.lanes.map((l) => l.uptime)}
                    highlight="uptime"
                  />
                ))}
              </div>
                ...
                <div className="space-y-1 flex-1">
                  {booking.devices.map((device) => (
                    <DeviceCell
                      ...
                      deviceId={device.id}
                      deviceName={device.name}
                      total={device.totalDowntime}
                      laneNames={device.lanes.map((l) => l.lane)}
                      laneValues={device.lanes.map((l) => l.downtime)}
                      highlight="downtime"
                    />
                  ))}
                </div>
