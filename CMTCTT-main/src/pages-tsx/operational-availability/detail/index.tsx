...
function UptimePill({ value }: { value: string }) {
  const zero = value === "0m 0s";
  return (
    <span
      className={cn(
        "inline-block font-mono text-xs px-2 py-0.5 rounded",
        zero ? "bg-gray-100 text-gray-400" : "bg-green-50 text-green-700 font-medium",
      )}
    >
      {value}
    </span>
...
function DowntimePill({ value }: { value: string }) {
  const zero = value === "0m 0s";
  return (
    <span
      className={cn(
        "inline-block font-mono text-xs px-2 py-0.5 rounded",
        zero ? "bg-gray-100 text-gray-400" : "bg-red-50 text-red-600 font-semibold",
      )}
    >
      {value}
    </span>
...
export function OperationalAvailabilityDetail({ monthId, bookingId, onNavigate }: Props) {
    ...
    <div className="flex-1 overflow-auto bg-gray-50">
      <div className="p-6">
        ...
        <div className="flex items-center gap-1.5 text-sm text-gray-400 mb-5 flex-wrap">
          ...
          <button
            onClick={() => onNavigate?.(`/operational-availability/${monthId}`)}
            className="hover:text-brand-primary transition-colors"
          >
            {record.month}
          </button>
        ...
        {booking.devices.map((device) => {
          type LaneRow = (typeof device.lanes)[number] & { _idx: number };

          const laneData: LaneRow[] = device.lanes.map((lane, idx) => ({
            ...lane,
            _idx: idx + 1,
          }));

          // Columns defined inline per-device — no hook dependency issue
          const laneColumns: ColumnDef<LaneRow, any>[] = [
            {
              id: "no",
              header: () => "#",
              cell: (info) => (
                <div className="text-xs text-gray-400">{info.row.original._idx}</div>
              ),
            },
            {
              id: "lane",
              header: () => "Lane",
              cell: (info) => (
                <div className="text-sm font-medium text-gray-700">{info.row.original.lane}</div>
              ),
            },
            {
              id: "uptime",
                ...
                return (
                  <span
                    className={cn(
                      ...
                      laneOk ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600",
                    )}
                  >
                    <span
                      className={cn(
                        "w-1.5 h-1.5 rounded-full",
                        laneOk ? "bg-green-500" : "bg-red-500",
                      )}
                    />
          ...
          return (
            <div key={device.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-4">
              {/* Device header */}
                  ...
                  <div>
                    ...
                    <p
                      className={cn(
                        ...
                        device.totalDowntime !== "0m 0s" ? "text-red-500" : "text-gray-400",
                      )}
                    >
