...
function ScanAssetsPage({ onBack }: { onBack: () => void }) {
  ...
  const stationColumns = useMemo<ColumnDef<BaseStationRow, any>[]>(
    () => [
      {
        id: "name",
      ...
      },
      {
        ...
        cell: (info) => (
          <div className="text-sm text-gray-700">{info.row.original.totalAssignments}</div>
        ),
      },
      {
        id: "issued",
        ...
        cell: (info) => (
          <div className="text-sm text-gray-700">{info.row.original.issued} asset(s)</div>
        ),
      },
      {
        id: "returned",
        ...
        cell: (info) => (
          <div className="text-sm text-gray-700">{info.row.original.returned} asset(s)</div>
        ),
      },
      {
        id: "missing",
        ...
        cell: (info) => (
          <div className="text-sm text-gray-700">{info.row.original.missing} asset(s)</div>
        ),
      },
      {
        id: "status",
        ...
        cell: (info) => (
          <span
            className={cn(
              ...
              info.row.original.status === "Missing"
...
export function AssetsAuditReport() {
  ...
  const auditColumns = useMemo<ColumnDef<AuditRecord & { _idx: number }, any>[]>(
    () => [
      {
        id: "no",
        header: () => "No",
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Button
            ...
            className="flex items-center w-fit gap-2 px-4 py-2 text-sm bg-brand-primary text-white rounded-md hover:bg-brand-primary-hover font-semibold"
          >
            Generate Report
          </Button>
        </div>
