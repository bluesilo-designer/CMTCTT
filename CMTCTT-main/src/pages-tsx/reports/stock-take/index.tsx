...
export function StockTakeVarianceReport({ onNavigate }: Props) {
  ...
  type IndexedRow = StockTakeRecord & { _idx: number };

  const paginated = filtered.slice((currentPage - 1) * PER_PAGE, currentPage * PER_PAGE);

  const indexedPaginated: IndexedRow[] = paginated.map((row, idx) => ({
    ...row,
    _idx: (currentPage - 1) * PER_PAGE + idx + 1,
  }));

  const columns = useMemo<ColumnDef<IndexedRow, any>[]>(
    () => [
      {
        id: "no",
        header: () => "No",
      ...
      },
      {
        ...
        cell: (info) => (
          <div
            onClick={() => onNavigate?.(`/reports/stock-take/${info.row.original.id}`)}
      ...
      },
      {
        ...
        header: () => "Scanned By",
        cell: (info) => (
          <div onClick={() => onNavigate?.(`/reports/stock-take/${info.row.original.id}`)} className="cursor-pointer">
