  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50">
        ...
        <div className="px-6 pt-6 pb-0">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="relative" ref={dlRef}>
              <Button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-brand-primary text-white text-sm font-semibold rounded-lg hover:bg-brand-primary-hover shadow-sm transition-colors w-auto"
              >
                <Download size={14} />
                Download
              ...
              {dlOpen && (
                <div className="absolute right-0 mt-1 w-52 bg-white border border-gray-200 rounded-xl shadow-xl z-10 overflow-hidden py-1">
          ...
          <div className="flex items-center gap-2 mb-4">
            <Button
              ...
              className="px-4 py-2 text-sm font-semibold border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors w-auto"
            >
          ...
          <div className="flex gap-1">
            {TABS.map((tab) => (
              <button key={tab} type="button" onClick={() => setActiveTab(tab)}
                className={cn(
                  "px-4 py-2.5 text-sm font-medium relative transition-colors whitespace-nowrap",
      ...
      {activeTab === "Training Performance" && (
        <TrainingPerformanceTab courseware={courseware} setCourseware={setCourseware} />
      )}
      {activeTab === "Nominal Roll" && (
        <NominalRollTab courseware={courseware} setCourseware={setCourseware} />
      )}
      {activeTab === "Detail List" && (
        <DetailListTab courseware={courseware} setCourseware={setCourseware} />
      )}
      {activeTab === "Leaderboard" && (
        <LeaderboardTab courseware={courseware} setCourseware={setCourseware} />
      )}
    </div>
  );
}
