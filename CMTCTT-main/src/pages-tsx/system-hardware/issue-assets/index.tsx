...
import type { IssueAssetsProps, Phase, ScannedAsset } from "./types";

export function IssueAssets({ onNavigate }: IssueAssetsProps) {
  ...
  useEffect(() => {
    ...
    if (!bs && !bk) {
      ...
      if (bsMatch) bs = decodeURIComponent(bsMatch[1]);
      const bkMatch = hash.match(/bookingIds=([^&]+)/);
      if (bkMatch) bk = decodeURIComponent(bkMatch[1]);
    }

    if (bs) setBaseStation(bs);
    if (bk) setBookingId(bk.split(",")[0]);
  }, []);
  ...
  const handleDeleteSelected = () => {
    setScannedAssets((prev) =>
      prev.filter((a) => !selectedIds.has(a.id)).map((a, i) => ({ ...a, no: i + 1 }))
    );
    setSelectedIds(new Set());
  };
  ...
  const handleStationChange = (id: string, station: string) => {
    setScannedAssets((prev) =>
      prev.map((a) => (a.id === id ? { ...a, targetBaseStation: station } : a))
    );
  };
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between flex-shrink-0">
        <div>
          <h2 className="text-sm font-bold text-gray-800">
            ...
            {phase === "scanned" && (
              <span className="ml-2 text-green-500 font-medium text-xs">● Connected</span>
            )}
          </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          {phase === "scanning" ? (
            <Button
              type="button"
              disabled
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-gray-100 text-gray-300 cursor-not-allowed"
            >
            ...
            <Button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-brand-primary text-white hover:bg-brand-primary-hover"
            >
        ...
        {phase === "scanning" ? (
          ...
          <div className="bg-white border border-gray-200 rounded-xl flex flex-col items-center justify-center py-20 px-6 text-center min-h-[400px]">
            <div className="mb-8 relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div
                  className="w-28 h-28 rounded-full border-2 border-red-300 animate-ping opacity-40"
              ...
              >
                ...
                <path
                  d="M 72 38 Q 82 47 82 56 Q 82 65 72 74"
                  stroke="#DC2626"
                  strokeWidth="2.5"
                  fill="none"
                  strokeLinecap="round"
                />
                <path
                  d="M 79 31 Q 93 42 93 56 Q 93 70 79 81"
                  stroke="#DC2626"
                  strokeWidth="2"
                  fill="none"
                  strokeLinecap="round"
            ...
            <div className="px-6 py-3 border-b border-gray-100 flex items-center justify-between">
              ...
              {selectedIds.size > 0 && (
                <Button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-50 text-red-600 border border-red-200 hover:bg-red-100"
                >
                  <Trash2 size={13} />
