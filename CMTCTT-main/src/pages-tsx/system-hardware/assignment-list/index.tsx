...
export function AssignmentList({ onNavigate }: { onNavigate?: (path: string) => void }) {
  ...
  const filtered = tabFiltered.filter(
    (a) =>
      !searchQuery || a.assignmentId.toLowerCase().includes(searchQuery.toLowerCase()),
  );
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            {returnCandidates.length > 0 && (
              <Button
                ...
                className="px-4 py-2 text-sm border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 font-semibold transition-colors bg-white"
              >
