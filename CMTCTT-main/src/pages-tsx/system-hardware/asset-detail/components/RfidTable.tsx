...
export function RfidTable({ asset, activeRfid }: RfidTableProps) {
  const rows: RfidRow[] = useMemo(() => {
    if (activeRfid === 1) {
      return [
        {
          name: "RFID Tag ID",
  ...
  const columns = useMemo<ColumnDef<RfidRow, any>[]>(
    () => [
      {
        id: "name",
        header: () => "Name",
        cell: (info) => (
          <div className="text-sm text-gray-600">{info.row.original.name}</div>
        ),
      },
      {
        id: "value",
      ...
      },
      {
        ...
        cell: (info) => (
          <div className="text-sm text-gray-400">{info.row.original.lastCheckedOn}</div>
        ),
      },
    ],
    []
