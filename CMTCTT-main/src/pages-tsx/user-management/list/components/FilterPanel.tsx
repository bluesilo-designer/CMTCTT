  ...
  return (
    <div className="absolute right-0 top-full mt-1.5 w-72 bg-white border border-gray-200 rounded-xl shadow-lg z-30 p-4 space-y-4">
      <div className="flex items-center justify-between mb-2">
        ...
        {hasActiveFilters && (
          <button onClick={onClearAll} className="text-xs text-brand-primary hover:underline">
            Clear all
          </button>
        )}
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {["Active", "Inactive"].map((s) => (
            <button
              key={s}
              onClick={() => onSetFilterStatus(filterStatus === s ? "" : s)}
              className={
                filterStatus === s
