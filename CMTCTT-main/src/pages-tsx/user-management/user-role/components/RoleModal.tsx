...
interface RoleModalProps {
  ...
  role?: UserRole;
  onClose: () => void;
  onConfirm: (name: string, status: "Active" | "Inactive") => void;
}

export function RoleModal({ mode, role, onClose, onConfirm }: RoleModalProps) {
  ...
  return (
    <Modal open={true} onClose={onClose} isUseX={false} width={448}>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Button
          onClick={() => { if (name.trim()) onConfirm(name.trim(), status); }}
          disabled={!name.trim()}
