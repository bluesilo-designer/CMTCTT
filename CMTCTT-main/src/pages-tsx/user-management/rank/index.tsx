...
export function RankPage() {
  ...
  const [modal, setModal] = useState<{ mode: ModalMode; rank?: Rank } | null>(null);
  ...
  const filtered = ranks.filter(
    (r) =>
      (!searchQuery || r.rank.toLowerCase().includes(searchQuery.toLowerCase())) &&
  ...
  const sorted = sortField ? sortBy(filtered, sortField as keyof Rank, sortDir) : filtered;
  ...
  const handleConfirm = (rankName: string, sequence: number, _status: "Active" | "Inactive") => {
    if (modal?.mode === "edit" && modal.rank) {
      setRanks((prev) =>
        prev.map((r) => (r.id === modal.rank!.id ? { ...r, rank: rankName, sequence } : r))
      );
  ...
  const columns = useMemo<ColumnDef<Rank, any>[]>(
    () => [
      {
        id: "select",
        width: 60,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div onClick={(e) => e.stopPropagation()}>
            <RowMenu
              onEdit={() => setModal({ mode: "edit", rank: row.original })}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {creators.map((creator) => (
                        <button
                          key={creator}
                          onClick={() =>
                            setFilterCreatedBy((prev) => (prev === creator ? "" : creator))
                          }
                          className={
                            filterCreatedBy === creator
              ...
              {filterCreatedBy && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-brand-primary/10 text-brand-primary text-xs font-medium border border-brand-primary/20">
                  ...
                  <button
                    ...
                    className="hover:text-brand-primary-hover ml-0.5"
                  >
                    ×
                  </button>
                </span>
