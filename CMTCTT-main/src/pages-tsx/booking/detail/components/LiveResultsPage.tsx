...
export function LiveResultsPage({ onBack }: { onBack: () => void }) {
  ...
  const traineeColumns = useMemo<ColumnDef<any, any>[]>(() => [
    { id: "no", header: () => "No", cell: ({ row }: any) => <span className="text-sm text-gray-700">{(page - 1) * RESULTS_PER_PAGE + row.index + 1}</span> },
