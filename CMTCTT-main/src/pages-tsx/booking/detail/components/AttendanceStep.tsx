...
import {
  ...
  Case2AttendanceSummaryModal,
  Case2UnregisteredAlert,
  Case2SuccessAlert,
} from "../modals/AlertModals";
...
function AttendanceStatusText({ status }: { status: string }) {
...
export function AttendanceStep({ onNext }: { onNext: () => void }) {
  const booking = useBookingStore((s) => s.booking);
  const [trainees, setTrainees] = useState<any[]>([]);
  const [scanVariant, setScanVariant] = useState<"case1" | "case2" | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [showUpload, setShowUpload] = useState(false);
  const [showScanPage, setShowScanPage] = useState(false);
  const [pendingVariant, setPendingVariant] = useState<"case1" | "case2">("case1");
  ...
  const startScan = (variant: "case1" | "case2") => { setPendingVariant(variant); setShowScanPage(true); };
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("lastUpdated", {
      ...
      cell: ({ getValue }) => (
        <span className="text-sm text-gray-400 whitespace-pre-line leading-snug">
    ...
    return (
      <ScanIDContent
        ...
        onFinished={(scanned) => {
          ...
          if (pendingVariant === "case1") setShowAttendanceConfirm(true);
        }}
      />
    );
  }
  ...
  if (showAttendanceConfirm) {
    return (
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        <AttendanceConfirmationModal open={true}
          attendeesCount={trainees.length}
          onConfirm={() => setShowAttendanceConfirm(false)}
          onCancel={() => { setTrainees([]); setShowAttendanceConfirm(false); setShowScanPage(false); }}
        />
      </div>
    );
  }
  ...
  if (showCase2FinalRoll) {
    return (
      <>
        <Case2FinalNominalRollPage
          trainees={trainees}
          onBack={() => setShowCase2FinalRoll(false)}
          onConfirm={() => { setShowCase2FinalRoll(false); setShowCase2Alert(true); }}
        />
        {showCase2Alert && (
          <Case2SuccessAlert open={showCase2Alert} onClose={() => { setShowCase2Alert(false); onNext(); }} />
        )}
      </>
    );
  }
  ...
  if (showFinalConfirm) {
    return (
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        <FinalConfirmationModal open={true}
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 flex flex-col md:flex-row md:items-center md:justify-between flex-shrink-0 gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button
            onClick={hasData ? () => isCase2 ? setShowCase2Summary(true) : setShowFinalConfirm(true) : undefined}
            disabled={!hasData}
            className={cn(
              "flex items-center gap-2 px-5 py-2.5 text-sm font-semibold w-auto",
              hasData ? "bg-brand-primary text-white hover:bg-brand-primary-hover" : "bg-gray-100 text-gray-400 cursor-not-allowed"
            )}
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative">
                ...
                <InputCustom
                  ...
                  className="pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg w-full sm:w-48 focus:ring-1 focus:ring-brand-primary"
                />
              </div>
              <Button
                type="outline"
              ...
              <button
                type="button"
                onClick={() => startScan("case2")}
                className="text-xs text-gray-400 underline underline-offset-2 hover:text-brand-primary transition-colors whitespace-nowrap"
              >
                Scan ID Case 2 →
              </button>
            </div>
      ...
      {showCase2Summary && (
        <Case2AttendanceSummaryModal open={showCase2Summary}
          trainees={trainees}
          onCancel={() => setShowCase2Summary(false)}
          onConfirm={() => { setShowCase2Summary(false); setShowCase2Unregistered(true); }}
        />
      )}
      {showCase2Unregistered && (
        <Case2UnregisteredAlert open={showCase2Unregistered}
          onCancel={() => setShowCase2Unregistered(false)}
          onUpdate={() => { setShowCase2Unregistered(false); setShowCase2FinalRoll(true); }}
        />
      )}
      {showCase2Alert && (
        <Case2SuccessAlert open={showCase2Alert} onClose={() => { setShowCase2Alert(false); onNext(); }} />
