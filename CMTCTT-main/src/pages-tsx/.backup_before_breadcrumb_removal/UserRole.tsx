...
import { userRoles } from "@/data/userManagement";
...
export function UserRole() {
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50">
      <div className="p-6">
        <div className="mb-4">
          <Breadcrumb items={["User Roles"]} />
        </div>

        <div className="flex items-center justify-between mb-5">
          <h1 className="text-xl font-semibold text-gray-800">
            User Role <span className="text-gray-400 font-normal text-base">({userRoles.length} Roles)</span>
          </h1>
              ...
              <tbody>
                {paginated.map((role, idx) => (
                  <tr key={role.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-sm text-gray-800">{role.name}</td>
                    ...
                    <td className="px-4 py-3.5 text-sm">
                      <div className="text-gray-800">{role.createdBy}</div>
                      <div className="text-gray-400 text-xs">{role.createdByRole}</div>
                    </td>
