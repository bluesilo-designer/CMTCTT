...
export function DeleteConfirmModal({ cabin, onClose, onConfirm }: Props) {
  return (
    <Modal open={true} onClose={onClose} width={420} isUseX={false}>
      <div className="flex items-start gap-4 mb-5">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">Delete Cabin</h2>
      ...
      <div className="bg-gray-50 rounded-lg px-4 py-3 mb-6 border border-gray-100">
        <p className="text-sm text-gray-500 mb-1">Cabin Name</p>
        <p className="text-sm font-semibold text-gray-800">{cabin.name}</p>
      </div>

      <p className="text-sm text-gray-600 mb-6">
        Are you sure you want to delete{" "}
        <span className="font-semibold text-gray-900">{cabin.name}</span>? This
      ...
      <div className="flex gap-3">
        ...
        <Button
          onClick={() => { onConfirm(cabin.id); onClose(); }}
