...
interface Props {
  cabin: Cabin;
  onClose: () => void;
  onSave: (id: string, name: string, status: CabinStatus) => void;
}

export function EditCabinModal({ cabin, onClose, onSave }: Props) {
  return (
    <Modal open={true} onClose={onClose} width={448} isUseX={false}>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">Edit Cabin</h2>
          <p className="text-sm text-gray-500 mt-0.5">Update the cabin details below.</p>
        </div>
      </div>

      <Formik
        initialValues={{ name: cabin.name, status: cabin.status }}
        validationSchema={schema}
        onSubmit={(values) => {
          onSave(cabin.id, values.name.trim(), values.status as CabinStatus);
          onClose();
        }}
      >
        {() => (
