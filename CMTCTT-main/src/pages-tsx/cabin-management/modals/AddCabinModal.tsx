...
const schema = Yup.object().shape({
  name: Yup.string().trim().required("Cabin name is required"),
  status: Yup.string()
...
interface Props {
  onClose: () => void;
  onAdd: (name: string, status: CabinStatus) => void;
}

export function AddCabinModal({ onClose, onAdd }: Props) {
  return (
    <Modal open={true} onClose={onClose} width={448} isUseX={false}>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">Add New Cabin</h2>
          <p className="text-sm text-gray-500 mt-0.5">Fill in the cabin details below.</p>
        </div>
      </div>

      <Formik
        initialValues={{ name: "", status: "Available" as CabinStatus }}
        validationSchema={schema}
        onSubmit={(values) => {
          onAdd(values.name.trim(), values.status as CabinStatus);
          onClose();
        }}
      >
        {() => (
          <Form>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Cabin Name <span className="text-brand-primary">*</span>
              </label>
              <Field
                name="name"
                type="text"
                placeholder="e.g. CTT-CLUSTER-45"
                autoFocus
