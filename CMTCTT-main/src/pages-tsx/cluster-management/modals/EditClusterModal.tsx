...
interface Props {
  cluster: Cluster;
  onClose: () => void;
  onSave:  (id: string, name: string, status: ClusterStatus) => void;
}

export function EditClusterModal({ cluster, onClose, onSave }: Props) {
  return (
    <Modal open={true} onClose={onClose} width={448} isUseX={false}>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">Edit Cluster</h2>
          <p className="text-sm text-gray-500 mt-0.5">Update the cluster details below.</p>
        </div>
      </div>

      <Formik
        initialValues={{ name: cluster.name, status: cluster.status }}
        validationSchema={schema}
        onSubmit={(values) => {
          onSave(cluster.id, values.name.trim(), values.status as ClusterStatus);
          onClose();
        }}
      >
        {() => (
