...
const DEFAULT_WEAPON_MAPPING: Record<
  Weapon,
...
export function ResourcePlanning() {
  ...
  const [stationLimits, setStationLimits] = useState<
    Record<Station, Record<Resource, number>>
  >({
  ...
  const [weaponMapping, setWeaponMapping] = useState<
    Record<Weapon, Partial<Record<Resource, number>>>
  >(DEFAULT_WEAPON_MAPPING);
  ...
  const handleWeaponMappingChange = (
    weapon: Weapon,
    resource: Resource,
    value: number,
  ) => {
      ...
      <div className="p-6 max-w-[1600px]">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-5 gap-3">
          ...
          {isEditing ? (
            <Button
              type="button"
              onClick={handleSave}
              className="flex items-center w-fit gap-2 px-5 py-2.5 bg-brand-primary text-white rounded-lg text-sm font-semibold hover:bg-brand-primary-hover transition-colors"
            >
              <Save size={16} /> Save
            </Button>
          ) : (
            <Button
              ...
              className="flex items-center w-fit gap-2 px-5 py-2.5 border border-gray-200 rounded-lg text-sm font-semibold text-gray-600 bg-gray-50 hover:bg-gray-100 transition-colors"
            >
              <Edit2 size={16} /> Edit
            </Button>
          )}
        ...
        <div className="mb-6 flex items-center gap-3 bg-white border border-gray-200 rounded-lg p-4">
          ...
          <select
            ...
            disabled={!isEditing}
            className={cn(
              "flex-1 max-w-xs px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 bg-white hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-transparent",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <div className="space-y-4">
              {(["pax", "ports", "igs", "monitors"] as Resource[]).map(
                (resource) => (
                  <div key={resource}>
                    ...
                    <InputCustom
                      ...
                      disabled={!isEditing}
                      className={cn(
                        "w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary",
                        !isEditing &&
                      ...
                      (resource) => (
                        <div key={resource} className="flex flex-col">
                          ...
                          <InputCustom
                            ...
                            disabled={!isEditing}
                            className={cn(
                              "w-full px-2 py-1 border border-gray-200 rounded text-xs focus:outline-none focus:ring-1 focus:ring-brand-primary",
                              !isEditing &&
                                "bg-gray-50 text-gray-600 cursor-not-allowed",
                            )}
                          />
